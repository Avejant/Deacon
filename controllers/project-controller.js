...
projectController.create = function(req, res) {
    Projects.findOne({$or:[{name:req.body.name}, {shortName : req.body.shortName}]}, function(err, project) {
    if (project) 
    {
        return res.send({nameError:project.name === req.body.name, shortNameError: project.shortName === req.body.shortName});
    }
...
projectController.update = function(req, res) {
    ...
    Projects.findById(req.params.id, function (err, project) {
        ...
        Projects.findOne({$and : [{$or:[{name:req.body.name}, {shortName : req.body.shortName}]},{_id:{$ne:project._id}}] }, function(err, sameProject) {
        if (sameProject) 
        {
            console.log("ADADA");
            return res.send({nameError:sameProject.name === req.body.name, shortNameError: sameProject.shortName === req.body.shortName});
        }
